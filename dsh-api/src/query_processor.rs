...
pub fn part_to_terminal_string(part: &Part) -> String {
  match part {
    Matching(part) => format!("\x1B[1m{}\x1B[0m", part),
    NonMatching(part) => format!("{}", part),
  }
}

pub fn parts_to_terminal_string(parts: &[Part]) -> String {
  parts.iter().map(|part| part_to_terminal_string(part)).collect::<Vec<_>>().join("")
}
